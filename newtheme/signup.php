...
if (!isset($_POST["userid"])) {
	echo('<div class="mainContent" id="mainContent" style="min-width:695px;"><a href="index.php" style="border:none;"><img id="meLogoTop" src="images/Me_Logo_Color.png"></a><h1 class="pageTitle">Error</h1><div class="subContent"><p>Please return to <a href="/">MeLibraries.ca</a>.</p></div></div>'); 
	include 'footer.php';
	exit();
}
...
?>
		<tr>
		<td>
			<a href="<?=$row['library_url']?>/" style="border:none;"><img src="libraries/<?=$row['record_index']?>.png" style="height:120px;vertical-align:middle;" alt="<?=$row['library_name']?>" title="<?=$row['library_name']?>"></a>
		</td>
		<td><form name="form<?=$row['library_record_index']?>" id="form<?=$row['library_record_index']?>" action="join.php" method="post">
				<input type="hidden" name="jsonField" id="jsonField" value="<?=htmlspecialchars ($_POST["jsonField"])?>" />
				<input type="hidden" name="userid" id="userid" value="<?=$_POST["userid"]?>" />
